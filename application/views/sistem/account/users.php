...
<div class="row">
	<div class="col-sm-10">
       <?= form_button('tambah', '<i class="fa fa-plus"></i> Tambah User' ,'id=bt_tambah_user class="btn btn-primary"')?>
...
<div class="pages_summary" id="u_summary"></div>       
<div id="u_pagination"></div>
    ...
    <div class="modal-dialog">
        <div class="modal-content animated flipInY">
            ...
            <div class="modal-body">
                <?= form_open('', 'id=formuser role=form class=form-horizontal'); ?>
                    ...
                    <div class="form-group add_pass">
                        ...
                        <div class="col-lg-8">
                          <label style="margin:7px;" id="label_password">1234 (Default)</label>
                        </div>
                    </div>
                <?= form_close(); ?>
            </div>
            <div class="modal-footer">
                ...
                <button type="button" id="bt_simpan" class="btn btn-primary" onclick="simpan_user()"><i class="fa fa-save"></i> Simpan</button>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    $(function(){
        ...
        $('#pegawai').select2({
            ajax: {
                url : '<?= base_url("api/masterdata_auto/pegawai_auto"); ?>',
                dataType : 'json',
                quietMillis : 100,
            ...
            formatResult: function(data){
                var markup = data.nama+'<br/><i><b>'+((data.jabatan!== null)?data.jabatan:'</b></i>');
                return markup;
            }, 
        ...
        $('#group').select2({
            ajax: {
                url: "<?= base_url('api/sistem_auto/group_user_auto') ?>",
                dataType: 'json',
                quietMillis: 100,
    ...
    function simpan_user() {
        ...
        if ($('#pegawai').val() === '') {
            dc_validation('#pegawai', 'Nama pegawai harus diisi!');
            stop = true;
        };

        if ($('#username').val() === '') {
            dc_validation('#username', 'Username harus diisi!');
            stop = true; 
        };

        if ($('#group').val() === '') {
            dc_validation('#group', 'Group harus dipilih!');
            stop = true;
        };
    ...
    function cek_username(username) {
        $.ajax({
            ...
            success: function (data) {
                if (data) {
                    dc_validation('#username', 'Username sudah dipakai, silahkan cari username lain');
