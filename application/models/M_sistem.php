...
class M_sistem extends CI_Model {
	...
	function get_list_group_user($limit, $start, $search){
        ...
        $sql = "select * from dm_group_users where id is not null $q";
        $order = " order by nama ";
    ...
    function get_group_user_privileges($id_group){
        $sql = "select p.*,m.nama as module, g.id_group_users from dm_privileges p
                join dm_module m on (m.id = p.id_module)
                left join dm_grant_privileges g on 
    ...
    function update_data_group_user($data){
        if ($data['id'] === false) {
            // insert
            $this->db->insert('dm_group_users', $data);
            $id = $this->db->insert_id();
        }else{
            // Update
            $id = $data['id'];
            $this->db->where('id', $data['id'])->update('dm_group_users', $data);
        }
    ...
    function update_data_group_user_privileges($data){
        $this->db->trans_begin();
        $this->db->where('id_group_users', $data['id_group'])->delete('dm_grant_privileges');

        // insert tabel dm_grant_privileges
        if (is_array($data['privileges'])) {
            foreach ($data['privileges'] as $key => $value) {
                ...
                $this->db->insert('dm_grant_privileges', $add);
            }
        }
    ...
    function delete_data_group_user($id){
        $this->db->where('id', $id)->delete('dm_group_users');
    }
    ...
    function get_user($id)
    {
        $sql = "select u.id, u.username, p.nama, 
                ...
                from dm_users u 
                join dm_pegawai p on (p.id = u.id) 
    ...
    function update_data_user($data , $tipe){
        if ($tipe === 'insert') {
            ...
            $this->db->insert('dm_users', $data);
        }else{
            // Update
            $this->db->where('id', $data['id'])->update('dm_users', $data);
        }
    ...
    function delete_data_user($id){
        $this->db->where('id', $id)->delete('dm_users');
    }
    ...
    function get_group_user($q, $start, $limit){
        $limit = " limit $start, $limit";
        $sql = "select * from dm_group_users where nama like ('%$q%') order by nama";
        $data['data'] = $this->db->query($sql.$limit)->result();
        $data['total'] = $this->db->query($sql)->num_rows();
        return $data;
    }
    ...
    function user_reset_password($id){
        ...
        $this->db->where('id', $id)->update('dm_users', $data);
    }

    function cek_username($username)
    {
        $sql = "select * from dm_users 
