...
function page_summary(total_data, total_datapage,limit, page){
    ...
    var str = '<div class="dataTables_info">Menampilkan '+start+' sampai '+finish+' dari '+total_data+' data</div>';
...
function dc_validation(element, pesan){
    $(element).next().remove();
    $(element).after('<div class="error" style="font-weight: normal;"><i>'+pesan+'</i></div>').closest('.form-group').removeClass('has-success').addClass('has-error');
}
